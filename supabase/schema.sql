...
create table if not exists public.budgets (
  ...
  amount       numeric(15, 2) not null check (amount >= 0),
  month        int not null check (month between 0 and 12),      -- 0 = template "semua bulan"
  year         int not null check (year = 0 or year between 2000 and 2100), -- 0 = template "semua bulan"
  created_at   timestamptz not null default now(),
